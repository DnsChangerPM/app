...
- سرویس VPN به‌صورت `foreground service` با نوع `specialUse` برای اندروید ۱۴+ ثبت شده و
  درخواست اجازه‌ی اعلان (Android 13+) هم به‌صورت خودکار انجام می‌شود.
- تمام API های حساس به نسخه (کانال اعلان، NetworkCallback، stopForeground و…) با `Build.VERSION` گارد شده‌اند.
